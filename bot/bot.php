...
if(preg_match("/\/start (?P<activation>[A-Za-z0-9]{20})/", $update["message"]["text"], $matches)) {
    if(!$result =$db->query("SELECT id FROM users WHERE  activation = '".$matches["activation"] . "'")) {
        ...
        if(!$result=$db->query("UPDATE `users` SET `chatid` = '". $update["message"]["chat"]["id"] . "', userid = '" . $update["message"]["from"]["id"] ."', first_name='".$update["message"]["from"]["first_name"] ."', activation='' WHERE `activation` = '".$matches["activation"]."';")) {
